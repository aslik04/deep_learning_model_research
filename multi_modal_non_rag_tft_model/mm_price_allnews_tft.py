# %% [markdown]
# ## Price + All News — Multimodal Non-RAG TFT (Henry Hub)
#
# Feature-engineered multimodal Temporal Fusion Transformer.
# All 91 FinBERT news features (12 buckets + Global, 7 metrics each).
...
EXPERIMENT = os.environ.get("EXP_NAME", "price_allnews")

# ------------------------------------------------------------------
# Run configuration
# ------------------------------------------------------------------
EXPERIMENT_NAME = "Price+AllNews"
MODEL_TAG = "tft"
...
NUM_COLS = []  # no numeric covariates beyond price for this variant
...
NEWS_COLS = (
    [f"{b}_{m}" for b in _ALL_BUCKETS for m in _METRICS]  # 84 bucket cols
    + [f"Global_{m}" for m in _METRICS]                    # 7 global cols  = 91
)
...
# ---- search space ----
# NOTE: Reduced batch/hidden choices to fit 92 features in 24GB GPU with FP16
ENCODER_CHOICES = [20, 30, 45, 60, 90, 120, 180]
BATCH_CHOICES   = [16, 32]  # Reduced from [32, 64, 128, 256] for memory
CLIP_CHOICES    = [0.1, 0.25, 0.5, 1.0, 2.0]
HIDDEN_CHOICES  = [32, 48, 64, 96, 128]  # Reduced from [..., 192, 256] for memory
...
def save_vsn_importances(model, val_loader, out_dir):
    ...
    try:
        # Get raw predictions WITHOUT return_x for interpret_output
        raw_preds = model.predict(val_loader, mode="raw", return_x=False)
