# %% [markdown]
# ## Price + Shuffled News — Multimodal Non-RAG TFT (Henry Hub)
#
# PERMUTATION CONTROL: identical to mm_price_global_tft.py but news rows are
# randomly shuffled before merging. If performance ≈ price-only baseline,
# the real news gains in other scripts are meaningful (not spurious).
...
EXPERIMENT = os.environ.get("EXP_NAME", "price_shuffled_news")

# ------------------------------------------------------------------
# Run configuration
# ------------------------------------------------------------------
EXPERIMENT_NAME = "Price+ShuffledNews"
MODEL_TAG = "tft"
...
def load_tft_ready_df(csv_path: str, news_path: str, news_cols: list) -> pd.DataFrame:
    ...
    # ── PERMUTATION CONTROL: shuffle news rows to destroy temporal alignment ──
    news_values = news[news_cols].values
    rng = np.random.RandomState(SEED)
    rng.shuffle(news_values)  # in-place row shuffle
    news[news_cols] = news_values
