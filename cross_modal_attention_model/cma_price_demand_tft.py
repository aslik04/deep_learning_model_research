# %% [markdown]
# ## Price + Demand News — Cross-Modal Attention Model (Henry Hub)
#
# Dual-encoder cross-modal attention with modality gating.
# Numeric stream (price only) queries news stream (28 demand-side FinBERT features).
...
EXPERIMENT_NAME = f"Price+DemandNews_CMA_{_attn_tag}{_gate_tag}"
MODEL_TAG = "cma"
...
NEWS_COLS = [f"{b}_{m}" for b in _DEMAND_BUCKETS for m in _METRICS]  # 28 cols
...
N_NEWS_FEATURES = len(NEWS_COLS)          # 28
