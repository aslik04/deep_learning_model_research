# %% [markdown]
# ## Price + All News — Cross-Modal Attention Model (Henry Hub)
#
# Dual-encoder cross-modal attention with modality gating.
# Numeric stream (price only) queries news stream (63 combined FinBERT features:
# 7 global + 28 demand + 28 supply).
...
EXPERIMENT_NAME = f"Price+AllNews_CMA_{_attn_tag}{_gate_tag}"
MODEL_TAG = "cma"
...
_DEMAND_BUCKETS = ["D1_DataCenters", "D2_Compute_Hw", "D3_Grid_Alerts", "D4_Weather_Extremes"]
_SUPPLY_BUCKETS = ["S1_Pipeline_Cap", "S2_LNG_Infra", "S3_Upstream", "S4_Supply_Shock"]
_METRICS = ["Vol", "Avg_Pos", "Avg_Neg", "Avg_Neu", "Max_Pos", "Max_Neg", "Dispersion"]
NEWS_COLS = (
    _GLOBAL_COLS
    + [f"{b}_{m}" for b in _DEMAND_BUCKETS for m in _METRICS]
    + [f"{b}_{m}" for b in _SUPPLY_BUCKETS for m in _METRICS]
)  # 7 + 28 + 28 = 63 cols

COVARIATE_COLS = NUM_COLS + NEWS_COLS

# Number of features per modality (used by the model to split encoder_cont)
N_NUMERIC_FEATURES = 1 + len(NUM_COLS)   # price only = 1
N_NEWS_FEATURES = len(NEWS_COLS)          # 63
...
def predict_and_denormalize(model, loader):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            preds_norm = model(x_numeric, x_news)

            center = x["target_scale"][:, 0:1].to(device)
            scale = x["target_scale"][:, 1:2].to(device)
            preds_real = preds_norm * scale + center
    ...
    actuals = np.concatenate(all_actuals, axis=0).reshape(-1)
...
def extract_interpretability_artifacts(model, test_loader, out_dir, test_dates=None):
    ...
    artifacts = {}
    if all_attn_weights:
        attn_raw = torch.cat(all_attn_weights, dim=0)
        attn_avg = attn_raw.mean(dim=1)
        artifacts["attn_weights_per_head"] = attn_raw
        artifacts["attn_weights_avg"] = attn_avg
    if all_gate_values:
        gate_vals = torch.cat(all_gate_values, dim=0)
    ...
    if all_gate_values:
        ...
        avg_gate_per_sample = gate_vals.squeeze(-1).mean(dim=1).numpy()
